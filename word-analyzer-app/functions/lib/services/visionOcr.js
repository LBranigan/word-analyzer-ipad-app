 ...
 */
function isHyphenLike(text) {
    return text === '-' || text === '—' || text === '–';
}
/**
 * Merge bounding boxes for multiple OCR words into one combined box
 */
function mergeBoundingBoxes(words) {
    if (words.length === 0) {
        return { x: 0, y: 0, width: 0, height: 0 };
    }
    if (words.length === 1) {
        return Object.assign({}, words[0].boundingBox);
    }
 ...
 *    - D-Day, V-Day, etc.
 */
function mergeHyphenatedCompounds(words) {
    const result = [];
    let i = 0;
