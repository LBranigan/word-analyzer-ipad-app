...
function calculateMetrics(matchingResult, audioDuration) {
    ...
    if (disfluencyRate <= 0.02)
        smoothnessPoints = 4; // Very smooth reading
    else if (disfluencyRate <= 0.05)
        smoothnessPoints = 3.5; // Smooth with occasional pause
    else if (disfluencyRate <= 0.10)
        smoothnessPoints = 3; // Some hesitation
    else if (disfluencyRate <= 0.20)
        smoothnessPoints = 2.5; // Noticeable hesitation
    else if (disfluencyRate <= 0.30)
        smoothnessPoints = 2; // Frequent hesitation
    else
        smoothnessPoints = 1.5; // Very choppy reading
    // Final prosody score (weights: 35% accuracy + 25% rate + 25% fluency + 15% smoothness)
    const prosodyScore = Math.round((accuracyPoints * 0.35 + ratePoints * 0.25 + fluencyPoints * 0.25 + smoothnessPoints * 0.15) * 10) / 10;
    // Grade assignment
    let prosodyGrade;
    if (prosodyScore >= 3.8)
        prosodyGrade = 'Excellent';
    else if (prosodyScore >= 3.0)
        prosodyGrade = 'Proficient';
    else if (prosodyScore >= 2.0)
...
function analyzeErrorPatterns(words) {
    ...
    const addPattern = (key, type, patternName, expected, spoken) => {
        const existing = patternMap.get(key);
        if (existing) {
            // Only add unique examples
            const alreadyHas = existing.examples.some(e => e.expected === expected && e.spoken === spoken);
            if (!alreadyHas) {
    ...
    for (const word of words) {
        ...
        if (expected[0] !== spoken[0]) {
            addPattern('initial_sound', 'initial_sound', 'Initial consonant substitution', word.expected, word.spoken);
        }
        // Final sound errors
        ...
        for (const [a, b] of visualPairs) {
            if ((expected.includes(a) && spoken.includes(b)) ||
                ...
                addPattern(`visual_${a}_${b}`, 'visual_similarity', `Visual confusion: ${a}/${b}`, word.expected, word.spoken);
            }
        }
        // Omission (spoken word is shorter - sounds/syllables left out)
        if (spoken.length < expected.length - 1) {
            addPattern('omission', 'omission', 'Sounds/syllables omitted', word.expected, word.spoken);
        }
        // Addition (spoken word is longer - extra sounds added)
        if (spoken.length > expected.length + 1) {
            addPattern('addition', 'addition', 'Extra sounds/syllables added', word.expected, word.spoken);
        }
        // Word length errors (errors on long words 7+ chars)
        if (expected.length >= 7) {
            addPattern('word_length_long', 'word_length', 'Difficulty with long words (7+ letters)', word.expected, word.spoken);
        }
        // Substitution tracking
        if (word.status === 'substituted') {
            addPattern(`sub_${expected}_${spoken}`, 'substitution', `"${word.expected}" → "${word.spoken}"`, word.expected, word.spoken);
        }
    }
