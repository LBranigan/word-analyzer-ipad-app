 ...
 */
async function generatePdfReport(input) {
    return new Promise((resolve, reject) => {
        ...
        const significantPatterns = input.errorPatterns.filter(p => p.count >= 2 &&
            !['hesitation', 'repetition', 'self_correction'].includes(p.type)).slice(0, 6);
        ...
        for (let i = 0; i < pages.count; i++) {
            ...
            doc
                ...
                .fontSize(8)
                .fillColor(COLORS.textMuted)
                .text('Generated by Word Analyzer', 50, 738, { width: contentWidth / 2, align: 'left' });
            doc
                .text(`Page ${i + 1} of ${pages.count}`, 50 + contentWidth / 2, 738, { width: contentWidth / 2, align: 'right' });
        }
        doc.end();
    });
}
