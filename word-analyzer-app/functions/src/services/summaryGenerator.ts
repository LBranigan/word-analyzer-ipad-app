...
import { AlignedWord } from './wordMatching';
import { Metrics, ErrorPattern, PatternSummary } from './metricsCalculator';
 ...
 */
function extractStrengths(words: AlignedWord[]): string[] {
 ...
 */
function extractStruggles(words: AlignedWord[]): StruggleWord[] {
  const struggles: StruggleWord[] = [];
 ...
 */
function extractPrimaryPattern(errorPatterns: ErrorPattern[]): SummaryInput['primaryPattern'] {
  ...
  if (skipAsPrimary.includes(errorPatterns[0].type) && errorPatterns.length > 1) {
    ...
    if (alternative) selectedPattern = alternative;
  }
 ...
 */
function generateCreativeSignoff(firstName: string, accuracy: number): string {
 ...
 */
function getRandomGreeting(firstName: string): string {
  const greetings = [
    `Hey ${firstName}!`,
 ...
 */
function generateFallbackSummary(studentName: string, metrics: Metrics, strengths: string[], struggles: StruggleWord[]): string {
  const firstName = studentName.split(' ')[0];
  const strengthWord1 = strengths[0] || '';
  const strengthWord2 = strengths[1] || '';
  const struggleWord = struggles[0]?.expected || '';
...
export async function generateAISummary(
  ...
  console.log(`Summary data: ${strengths.length} strengths, ${struggles.length} struggles, pattern: ${primaryPattern?.type || 'none'}`);
